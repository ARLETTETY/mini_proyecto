...
from src.schemas.holiday import FeriadoCreate, FeriadoUpdate, FeriadoResponse
...
@router_holiday.get("/", response_model=list[FeriadoResponse])
def read_feriados(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return get_feriados(db, skip, limit)

@router_holiday.get("/{holiday_id}", response_model=FeriadoResponse)
...
@router_holiday.post("/", response_model=FeriadoResponse, status_code=201)
def create_new_feriado(feriado: FeriadoCreate, db: Session = Depends(get_db)):
    return create_feriado(db, feriado)

@router_holiday.put("/{holiday_id}", response_model=FeriadoResponse)
